...
public class Code03_Comparator {
    ...
    public static void main(String[] args) {
        ...
        treeSet1.add(new Employee(2, 27)); // 会去重
        System.out.println(treeSet1.size());
        ...
        /**
         * A[开始]：进入比较逻辑。
         * B{公司编号是否相同?}：判断两个员工的公司编号是否相同。
         * 如果不同，进入节点 C。
         * 如果相同，进入节点 D。
         *
         * C[按公司编号排序]：根据公司编号进行排序。
         * D{年龄是否相同?}：判断两个员工的年龄是否相同。
         * 如果不同，进入节点 E。
         * 如果相同，进入节点 F。
         *
         * E[按年龄排序]：根据年龄进行排序。
         * F[按toString()排序]：根据对象的字符串表示进行排序。
         * G[结束]：结束比较逻辑。
         */
        TreeSet<Employee> treeSet2 = new TreeSet<>((a, b) -> a.company != b.company ? (a.company - b.company) : a.age != b.age ? (a.age - b.age) : a.toString().compareTo(b.toString()));
        for (Employee e : arr) {
            treeSet2.add(e);
        }
